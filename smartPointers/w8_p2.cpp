...
int main(int argc, char *argv[]) {
...

  {
    ...
    std::cout << (db1 == db2 ? "Same" : "Different")
              << " database.\n"; // Different
    std::cout << (db1 == db4 ? "Same" : "Different")
              << " database.\n"; // Different
  }
...

  {
    ...
    for (auto &key : keys) {
      auto dB = sdds::Database<std::string>::getInstance(
          key); // the parameter should be ignored
      std::string value;
    ...
    auto dB = sdds::Database<std::string>::getInstance("");
    if (dB->SetValue("Leylah Fernandez", "$786,772") ==
        sdds::Err_Status::Err_Success) {
      std::string value;
      if (dB->GetValue("Leylah Fernandez", value) ==
          sdds::Err_Status::Err_Success)
...
void dumpFileAsHex(const char *filename) {
  ...
  std::cout << "---------------------------------------------------------------"
               "------\n";
  std::cout << "File size in bytes: [ " << file.tellg() << std::setfill('0')
            << std::hex;
